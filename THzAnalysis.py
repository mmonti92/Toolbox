...
class THzAnalyser(object):
        ...
        def __init__(self, fileList, refList,
                     ...
                     fitQty='Conductivity', thin=True, sigma_b=0.0):
                """
                Parameters
                ----------
                fileList: list of str
                ...
                if fmt == 'Ox':
                    tmp_xList, tmp1_EtList = self.Data_Reader(fileList, fmt,
                                                              6, shape)
                    tmp_xList, tmp2_EtList = self.Data_Reader(fileList, fmt,
                                                              5, shape)

                    tmp_xRefList, tmp1_EtRefList = self.Data_Reader(refList,
                                                                    fmt,
                                                                    4, shape)
                    tmp_xRefList, tmp2_EtRefList = self.Data_Reader(refList,
                                                                    fmt,
                                                                    3, shape)
                        ...
                        if multipleFit:
                            for i, o in enumerate(y):
                                ...
                                self.multiParams.append(
                                    self.Fit(x=self.fList[i,
                                             int(boundaries[i][0]):
                                             int(boundaries[i][1])],
                                             y=o[int(boundaries[i][0]):
                                                 int(boundaries[i][1])],
                                             model=model,
                                             err=None,
                                             init=init[i], para=self.para,
                                             c=complexFunction, guess=guess,
                                             plot=plot,
                                             fitQty=fitQty))
                        elif not multipleFit:
                            # print(int(boundaries[1]))
                            self.params = self.Fit(x=self.f[int(boundaries[0]):
                                                            int(boundaries[1]
                                                                )],
                                                   y=y[int(boundaries[0]):
                                                       int(boundaries[1])],
                                                   model=model,
                                                   err=[err[0]
                                                   [int(boundaries[0]):
        ...
        def Data_Computation(E, ERef, x, xRef,
                             sample, fmt='', flip=False, exp=0,
                             window='', para=[], thin=True, sigma_b=0.0):
                """
