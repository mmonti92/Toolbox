...
import DataAnalysis.MyTools as man
...
class THzAnalyser(object):
    ...
    def __init__(
        ...
        if model == "Drude":
            self.para = {"mr": self.sample.massEffRatio}
        ...
        lenfft = np.int(np.round((exp + 1) * (lenFiles - stop - start) / 2 + 0.6))
        listShape = (numFiles, (exp + 1) * (lenFiles - stop - start))
        lenT = listShape[1]
        ...
        self.fList = np.zeros(listShapeFft, dtype=np.complex_)  # Frequency
        self.EList = np.zeros(listShapeFft, dtype=np.complex_)  # Complex FFT field
        self.ERefList = np.zeros(
            listShapeFft, dtype=np.complex_  # Complex FFT
        )  # Reference field
        self.transList = np.zeros(listShapeFft, dtype=np.complex_)  # Transmission
        self.sigmaList = np.zeros(listShapeFft, dtype=np.complex_)  # Conductivity
        self.epsilonList = np.zeros(
            listShapeFft, dtype=np.complex_  # Dielectric
        )  # function
        self.lossList = np.zeros(listShapeFft, dtype=np.complex_)  # Loss  # Function
        self.nList = np.zeros(
            listShapeFft, dtype=np.complex_  # Complex refractive
        )  # index

        # Averaged frequency domain arrays
        self.f = np.zeros(lenfft, dtype=np.complex_)  # Same quantities  # but averaged
        self.E = np.zeros(lenfft, dtype=np.complex_)
        self.ERef = np.zeros(lenfft, dtype=np.complex_)
        self.sigma = np.zeros(lenfft, dtype=np.complex_)
        self.trans = np.zeros(lenfft, dtype=np.complex_)
        self.epsilon = np.zeros(lenfft, dtype=np.complex_)
        self.loss = np.zeros(lenfft, dtype=np.complex_)
        self.n = np.zeros(lenfft, dtype=np.complex_)
        ...
        tmp_xList, tmp_EtList = self.Data_Reader(fileList, fmt, sigCol, shape)
        tmp_xRefList, tmp_EtRefList = self.Data_Reader(refList, fmt, refCol, shape)

        if fmt == "Ox":
            tmp_xList, tmp1_EtList = self.Data_Reader(fileList, fmt, 6, shape)
            tmp_xList, tmp2_EtList = self.Data_Reader(fileList, fmt, 5, shape)

            tmp_xRefList, tmp1_EtRefList = self.Data_Reader(refList, fmt, 4, shape)
            tmp_xRefList, tmp2_EtRefList = self.Data_Reader(refList, fmt, 3, shape)
            tmp_EtList = tmp1_EtList - tmp2_EtList
            tmp_EtRefList = tmp1_EtRefList - tmp2_EtRefList
        ...
        for i in range(lenfft):
            ...
            self.sigmaUncReal[i] = np.std(np.real(self.sigmaList[:, i]))
            self.sigmaUncImag[i] = np.std(np.imag(self.sigmaList[:, i]))
            self.transUnc[i] = np.std(np.abs(self.transList[:, i]))
            self.epsilonUncReal[i] = np.std(np.real(self.epsilonList[:, i]))
            self.epsilonUncImag[i] = np.std(np.imag(self.epsilonList[:, i]))
        ...
        #                                            np.real(self.sigma))
        if fitFlag:
            y_Map = {"Conductivity": self.sigma, "Transmission": self.trans}
            if multipleFit:
                y_Map = {"Conductivity": self.sigmaList, "Transmission": self.transList}
            y = y_Map[fitQty]
            err_Map = {
                "Conductivity": [self.sigmaUncReal, self.sigmaUncImag],
                "Transmission": [self.transUnc, np.zeros(len(self.transUnc))],
            }
            err = err_Map[fitQty]
            if model == "":
                model = self.sample.f
                wn.warn(
                    "Warning:: model undefined, sample" + "'s default chosen: " + model,
                    RuntimeWarning,
                )
            if multipleFit:
                for i, o in enumerate(y):
                    ...
                    self.multiParams.append(
                        self.Fit(
                            x=self.fList[
                                i, int(boundaries[i][0]) : int(boundaries[i][1])
                            ],
                            y=o[int(boundaries[i][0]) : int(boundaries[i][1])],
                            model=model,
                            err=None,
                            init=init[i],
                            para=self.para,
                ...
                # print(int(boundaries[1]))
                self.params = self.Fit(
                    x=self.f[int(boundaries[0]) : int(boundaries[1])],
                    y=y[int(boundaries[0]) : int(boundaries[1])],
                    model=model,
                    err=[
                        err[0][int(boundaries[0]) : int(boundaries[1])],
                        err[1][int(boundaries[0]) : int(boundaries[1])],
                    ],
                    init=init,
                    para=self.para,
                    c=complexFunction,
        ...
        if plot:
            (self.multifig, self.multitimefig, self.finalfig) = self.Data_Plotter(fmt)

            self.valuesfig = 0

    def __str__(self):
        return str(self.fileList) + " " + str(self.sample) + " " + str(self.params)
    ...
    def Data_Reader(fileList, fmt, col=0, shape=0):
        ...
        if shape == 0:
            data = man.Reader(fileList, delimiter=delm, skip_header=sr)
            return data
        elif shape != 0:
            data = np.zeros((len(fileList), shape[0], shape[1]))

            for i, file in enumerate(fileList):
                data[i] = man.Reader(file, delimiter=delm, skip_header=sr)
    ...
    def Data_Computation(
        ...
        if window != "":
            (
                x,
                E,
            ) = man.Window(window, [p * cv for p in para], x, E)
            (
                xRef,
                ERef,
            ) = man.Window(window, [para[0] * cv, para[1] * cv], xRef, ERef)
        if exp > 0:
        ...
        fac = 1.0
        if sigma_b > 0.0:
            k = c0 / (np.real(freq) * 1e12)
            fac = 1 / (1 + k**2 / sigma_b**2)

        trans = Efft / EReffft
        if thin:
            sigma = -(ns + n2) * (Efft - EReffft) / (Z0 * d * EReffft) / fac
            # sigma = -(ns + n2) * (Efft -
            #                       EReffft) / (Z0 * d * Efft) /\
            #     fac
        elif not thin:
            ...
            #           (n**2 + 1)) * dEonE / (1 + dEonE)
            sigma = -(ns + n2 / (Z0 * d)) * dEonE / (1 + dEonE)
        ...
        n = np.sqrt(epsilon)
        return (t, E, tRef, ERef, freq, Efft, EReffft, trans, sigma, epsilon, loss, n)
    ...
    def Data_Plotter(self, fmt=""):
        ...
        fig, ((ax0, ax1), (ax2, ax3)) = plt.subplots(
            nrows=2, ncols=2, sharex="all", squeeze=True
        )
        for s in self.sigmaList:
            i = np.where(s == self.sigmaList)[0][0]
            (idxMin, idxMax, reCMin, reCMax) = man.Extrema(
                fMin, fMax, self.fList[i], np.array((np.real(s),))
            )
            (idxMin, idxMax, imCMin, imCMax) = man.Extrema(
                fMin, fMax, self.fList[i], np.array((np.imag(s),))
            )
            plot0 = ax0.plot(np.real(self.fList[i]), np.real(s), ls="-")
            color = plot0[0].get_color()
            ax0.plot(np.real(self.fList[i]), np.imag(s), ls="--", color=color)
            ax0.set_ylim(min(reCMin, imCMin), max(reCMax, imCMax))

            (idxMin, idxMax, reCMin, reCMax) = man.Extrema(
                fMin, fMax, self.fList[i], np.array((np.real(self.epsilonList[i]),))
            )
            (idxMin, idxMax, imCMin, imCMax) = man.Extrema(
                fMin, fMax, self.fList[i], np.array((np.imag(self.epsilonList[i]),))
            )
            plot1 = ax1.plot(
                np.real(self.fList[i]), np.real(self.epsilonList[i]), ls="-"
            )
            color = plot1[0].get_color()
            ax1.plot(
                np.real(self.fList[i]),
            ...
            (idxMin, idxMax, reCMin, reCMax) = man.Extrema(
                fMin, fMax, self.fList[i], np.array((np.abs(self.transList[i]),))
            )
            ax2.plot(np.real(self.fList[i]), np.abs(self.transList[i]), ls="-")
            ...
            if fmt == "TW":
                specToPlotOFF = np.abs(self.ERefList[i] * 1j * self.fList[i])
                specToPlotON = np.abs(self.Elist[i] * 1j * self.fList[i])
            if fmt == "":
                wn.warn(
                    "Warning:: undefined or wrong"
            ...
            ax3.semilogy(
                np.real(self.fList[i]), specToPlotON * 1e3, color=color, ls="-"
            )
            ax3.set_ylim(1e-4, 1)
        ...
        for e in self.EtList:
            ...
            plot0 = ax0.plot(
                self.tList[i], e / max(self.EtRefList[i]), ls="-", label=str(i)
            )
            color = plot0[0].get_color()
            ax0.plot(
                self.tRefList[i],
            ...
            ax0.plot(
                self.tRefList[i],
                (self.EtRefList[i] - e) / max(self.EtRefList[i]),
                color=color,
                ls="-.",
            )
        if self.window != "":
            ax0.plot(self.t, mt.Function("Gauss", self.para, self.t))
        ax0.set_ylabel("E$_{\mathrm{THz}}$(a.u.)")
        ax0.set_xlabel("t(ps)")
        ax0.legend(loc="best")
        finalfig, (ax0, ax1) = plt.subplots(
            nrows=1, ncols=2, sharex="all", squeeze=True
        )

        (idxMin, idxMax, reCMin, reCMax) = man.Extrema(
            fMin, fMax, self.f, np.array((np.real(self.sigma),))
        )
        (idxMin, idxMax, imCMin, imCMax) = man.Extrema(
            fMin, fMax, self.f, np.array((np.imag(self.sigma),))
        )
        ax0.errorbar(
            np.real(self.f), np.real(self.sigma), self.sigmaUncReal, marker="o", ls=""
        )
        ax0.set_xlim(fMin, fMax)
        ...
        ax1.errorbar(
            np.real(self.f), np.imag(self.sigma), self.sigmaUncImag, marker="o", ls=""
        )
    ...
    def Fit(
        ...
        yLabel_Map = {"Conductivity": "$\sigma$", "Transmission": "T"}
        ...
        if init:
            if model == "Drude":
                ...
                par.add("fC", init["fC"])
                paras = []
            elif model == "DoubleCyclotronTransmission":
                par.add("A", init["A"])
                par.add("gamma", init["gamma"])
                par.add("fC", init["fC"])
                par.add("B", init["B"])
                par.add("gamma2", init["gamma2"])
                par.add("fC2", init["fC2"])
                paras = []
        ...
        out = fit.minimize(res, par, args=(x,), kws={"data": data}, nan_policy="omit")
        ...
        if plot:
            ...
            if c:
                ...
                col = 1
            fitFig, axes = plt.subplots(nrows=1, ncols=col, sharex="all", squeeze=True)

            if c:
                ebar = axes[0].errorbar(x, np.real(y), np.real(err), marker="o", ls="")
                c0 = ebar[0].get_color()
                axes[0].plot(x, np.real(fitted), ls="-", marker="", color=c0)
                if guess:
                    axes[0].plot(x, np.real(guessed), ls="--", marker="", color=c0)
                axes[1].errorbar(
                    x, np.imag(y), np.imag(err), marker="o", ls="", color=c0
                )
                axes[1].plot(x, np.imag(fitted), ls="-", marker="", color=c0)
                if guess:
                    axes[1].plot(x, np.imag(guessed), ls="--", marker="", color=c0)
                axes[0].set_xlabel("$\\nu$(THz)", x=1.05)
                axes[0].set_ylabel(yLabel)
                axes[1].yaxis.set_label_position("right")
                axes[1].tick_params(which="major", right=True, left=False)
                axes[1].yaxis.set_ticks_position("right")

            elif not c:
                # print(len(x), len(y))
                ebar = axes.errorbar(x, np.real(y), np.real(err), marker="o", ls="")
                c0 = ebar[0].get_color()
                axes.plot(x, np.real(fitted), ls="-", marker="", color=c0)
                if guess:
                    axes.plot(x, np.real(guessed), ls="--", marker="", color=c0)
                axes.set_xlabel("$\\nu$(THz)")
